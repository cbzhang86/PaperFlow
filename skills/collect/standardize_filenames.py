...
if not PDF_DIR.exists():
    PDF_DIR = PROJECT_ROOT / "01_raw" if (PROJECT_ROOT / "01_raw").exists() else PDF_DIR

import fitz

# ── arXiv ID → 标题映射（新增论文可维护此表）──
_ARXIV_MAP = {
...
def extract_data(text: str, meta: dict) -> dict:
    ...
    # 作者
    for l in lines[:25]:
        m = re.search(r'^([一-鿿]{2,4})[；;][一-鿿]{2,4}[[（(]?\d', l)
        if m and len(l) < 60: result["author"] = m.group(1); break
        m = re.search(r'^([一-鿿]{2,4})[[（(（(]\d', l)
        if m and len(l) < 60: result["author"] = m.group(1); break
    if not result["author"]:
        for l in lines[:15]:
            if re.match(r'arXiv:', l): continue
            ...
            if m and len(l) < 60 and not re.search(r'\b(Abstract|Department|University|Institute|Research|Laboratory|School|College)\b', l):
                ...
                parts = name.split()
                if len(parts) >= 2: result["author"] = parts[-1].rstrip('.,; '); break
    if not result["author"]:
        for l in lines:
            m = re.search(r'作者简介[：:]\s*([一-鿿]{2,4})', l)
            if m: result["author"] = m.group(1); break
    if not result["author"]:
        for l in lines[:15]:
            pair = re.match(r'^([一-鿿]{2,4})[　 ]+([一-鿿]{2,4})$', l)
            if pair and len(l) < 25: result["author"] = pair.group(1); break
    if not result["author"]:
        for l in lines:
...
def build_new_name(f: Path):
    ...
    parts = []
    if data["year"]: parts.append(data["year"])
    if data["author"]: parts.append(data["author"])
    if title:
        ...
        if len(safe) > 80:
            ...
            last_u = safe.rfind('_')
            if last_u > 50: safe = safe[:last_u]
        parts.append(safe)

    if not parts: return None
    new_name = "_".join(parts) + ".pdf"
    if new_name == orig_name: return None
    return (f, new_name)


def cmd_scan():
    changes = [r for f in sorted(PDF_DIR.rglob("*.pdf")) if (r := build_new_name(f))]
    if not changes: print("✅ 所有文件名已符合标准"); return
    ...
    for old, new_name in changes:
        ...
        if new_path.exists():
            ...
            while new_path.exists():
                new_path = old.parent / f"{new_path.stem}_{c}.pdf"; c += 1
        if dry_run:
            print(f"  [{old.parent.name}] {old.name} → {new_path.name}")
            continue
        try:
            old.rename(new_path); ok += 1
            print(f"  ✅ {old.parent.name}/ {old.name} → {new_path.name}")
        except Exception as e:
            errs.append((old.name, str(e))); print(f"  ❌ {old.name}: {e}")
...
if __name__ == "__main__":
    ...
    elif args.cmd == "fix": cmd_fix(dry_run=args.dry_run)
