...
# 可选 Zotero DB 检测（无 Zotero 时跳过）
ZOTERO_DB = None
try:
    from global_config import ZOTERO_DB as _ZOTERO_DB
    ZOTERO_DB = _ZOTERO_DB
...
def make_standard_filename(paper: dict, max_title_len: int = 80) -> str:
    ...
    所有采集 download() 统一通过此函数生成文件名，不再各自为政。

    Args:
        paper: 论文信息字典，应含 {title, year, author, ...}
        max_title_len: 标题段最大长度
    Returns:
        标准文件名，如 '2026_吴玉韶_十五五时期我国养老服务体系治理逻辑.pdf'
    """
...
class BaseSource(ABC):
    ...
    def search(self, keyword: str, limit: int = 5) -> list:
        """搜索论文，返回 [{title, url, source, year?, author?}]"""
        pass
