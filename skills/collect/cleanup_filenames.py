...
def scan_files() -> list:
    ...
    for subdir in sorted(PDF_DIR.iterdir()):
        ...
        for f in sorted(subdir.glob("*.pdf")):
            ...
            if issues_found:
                issues.append({
                    "path": f, "dir": subdir.name,
                    "current_name": name, "issues": issues_found,
                })
    return issues
...
def cmd_fix(dry_run: bool = False):
    ...
    for item in issues:
        ...
        if dry_run:
            print(f"  [试运行] {f.relative_to(PDF_DIR)} → {final_path.relative_to(PDF_DIR)}")
            continue
        ...
        if final_path.exists():
            ...
            while final_path.exists():
                final_path = final_dir / f"{stem}_{cnt}.pdf"
                cnt += 1
        shutil.move(str(f), str(final_path))
        if target_dir_name: moved += 1
        if new_name != item["current_name"]: renamed += 1
    if not dry_run:
